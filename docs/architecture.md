...
`<monitorDir>/events/<YYYY-MM-DD>.jsonl` は**既定 30 日**で切る
（`EVENTS_KEEP_DAYS`、`serve --events-keep-days N`、`0` で無効）。実測で
**約 7MB/日**積み上がり、誰も消していなかった。30 日にしたのは Claude Code 自身が
transcript を保持する窓と同じだからで、それより古い日の hook 履歴は
transcript 側も消えているので突き合わせる相手がいない。

削除は collector の**起動時と日跨ぎ時だけ**（1秒ティックで readdir はしない）。
対象は自分のディレクトリ配下で `^\d{4}-\d{2}-\d{2}\.jsonl$` に完全一致する名前のみ。
削除件数は `stats().eventFilesDeleted` に出る。

日跨ぎの順序は**「読んでから切る」**。`activeDates` は Set なので挿入順で回ると
00:30 起動（today→yesterday の順で挿入）の翌日の切り詰めで**「たった今終わった日」が
先に消え、2日前が残る**。しかも切り詰めが読みの前だったので、23:59:59.8 に書かれた
`SessionEnd` を確実に落としていた。いまは (1) 新しい日を足す → (2) 全対象日を読む →
(3) 日付を**ソートして古い方から**2件に切り詰める、の順にしてある。

Tree 側（`HookHistory`）も全日読みをやめ、**開くセッションの期間 ±1 日**だけを開く
（`datesForSpan`）。期間が判らなければ直近 7 日にフォールバックする。
±1 日の余白は、hook が**ローカル日付**でファイルを分けるのに transcript の
timestamp が UTC で、日跨ぎ付近で両者の日付がずれるため。
...
（`a1a1a1a1a1a1a1a1a`: `PreToolUse` と `PostToolUse` が1件ずつだけで、
...
永久に来ない。実測: `11111111` の最後のイベントは 2026-09-02T23:59:50 の
...
6. **PID再利用。** `procStart` と実プロセスの起動時刻を突き合わせて検知するが
   ...
   ことが多い）。起動時刻を取得できなかった pid は前回の判定を維持する（記憶が無ければ
   `kill(0)` の結果をそのまま返す）。
   突合の基準はプラットフォームで違う。**Windows** は `procStart` を正とし、
   `startedAt` は `procStart` が無いときだけのフォールバックとして使う
   （`--resume` や起動の遅れで `startedAt` は正当にズレるので、ハード条件に
   してはならない）。**Linux** はティック同士を整数で比較する（60 秒 = 6000
   ティックの許容幅）。btime を経由しないので、NTP 補正や WSL2 のスリープ復帰に
   よる時計のステップ補正があっても誤判定しない。ただしティックは起動ごとに
   振り出しに戻るため、前回起動の古いファイルが今回起動の若いプロセスと偶然
   一致することがある（WSL で実測: procStart 3594 の 4 か月前のファイルと、
   779 ティックで起動した bash）。そこで**起動跨ぎの保険**を併用する——
   `startedAt` が btime より 10 分以上前なら、ティックが一致していても再利用と
   見なす（10 分は時計のステップ補正で btime が前にずれたときに、起動直後の
   本物のセッションを落とさないための余裕）。ティックが読めないときだけ
   `startedAt` と実測起動時刻の絶対比較にフォールバックする。
...
11. **PID再利用の検査は60秒に1回。** `sessions.js` の突合は PowerShell を
    ...
    58 秒は稼働中として表示される（WSL で実測）。記憶は生きているセッション
    全部に**先に**適用し、そのうえで起動時刻を実際に読めた pid の分だけ
    上書きする。だから検査したのに起動時刻が読めなかった pid も前回の判定を
    保つ。記憶を捨てるのは `kill(0)` が死んだと言ったときと、今回の一覧に
    出てこなかったキー（ファイルが消えた＝正常終了、または書き換わった）で、
    同じ PID に来た新しいセッションが古い判定を引き継ぐことはない。
...
23. **モデル名の表が実データに追いついていなかった。** 2026-09-06 に実 transcript を
    ...
    **2026-09-25 追記（Issue #3）:** `claude-opus-5-5` が同じ理由で `other` に落ちた。
    表の足し忘れを繰り返さないよう、両方の表引きに**系列接頭辞のフォールバック**を
    入れた。表に無い ID でも `claude-<系列>-...`（系列は opus / sonnet / haiku /
    fable の4つだけ）なら、その系列に写す。末尾の `[1m]` などは無視する。
    知らない系列名や `<synthetic>` は従来どおり `other`（ツリーでは素通し）。
    表は残してあるが、今後は新しい版が出ても**追記しなくてよい**。
    写し方が変わると、保存ストアの過去日は合計が同じまま内訳だけが古くなる。
    そこで、合計が同じでも系列別の内訳が違う日は `daily.json` を書き直す
    （合計が減った日は従来どおり書き直さない）。そのため、transcript が既に一部
    または全部消えた日は古い内訳（`other`）のまま残る。単調マージを崩さないための
    割り切りで、直すなら `daily.json` を手で消す（制約25）。
...
### 公開前レビュー（バッチA）で新たに判明した制約

35. **保持日数を超えた hook 履歴は Tree で jsonl 推定にフォールバックする。**
    `events/` を既定 30 日で切る（4.3）ので、それより古いセッションを Tree で
    開くと `SubagentStop` などの hook 証拠が存在せず、エージェントの状態は
    transcript からの推定（`async-unknown` や `jsonl:*`）になる。`statusSource`
    にそう出るので嘘にはならないが、**同じセッションを 30 日の前後で開くと
    表示が変わる**。長期保存が要るなら `--events-keep-days` を伸ばすこと
    （ディスクは実測 約7MB/日）。
36. **`HookHistory` は「聞かれた日」しか開かない。** 期間の判らないセッション
    （transcript に timestamp が無く mtime も取れない）は直近 7 日にしか
    フォールバックしないので、それより古い hook 証拠は読まれない。上と同じく
    jsonl 推定に落ちる。
37. **相対パスの `CLAUDE_MONITOR_DIR` はホーム基準で解決する。** サーバは
    起動した cwd、hook は Claude Code のプロジェクト cwd で動くので、cwd 基準に
    すると**両者が別のディレクトリを見てダッシュボードが永久に空になる**。
    `src/paths.js` と `hooks/*.js` の3箇所で同じ規則にしてある。絶対パスは
    そのまま。
38. **`JsonlTail.read()` は複数チャンクの途中で失敗しうる。** 2チャンク目以降の
    `readSync` が失敗した場合、そこまでの行は返し、オフセットもそこで止める
...
transcript のレコードは**時刻順に並んでいない**（実測 44444444: 5行目が .266Z、
...
当時の別の版を指していた可能性がある）。テーブルに無いIDは、`claude-<系列>-...`
の形で系列が4つのどれかなら系列名に写し（Issue #3、制約23の追記を参照）、
それ以外は 4.6 の規則どおりそのまま出す（勝手に整形しない）。

### 6.3 hooks にしか存在しないエージェント

`subagents/` にファイルが無く meta.json も無いのに hooks は知っている、という
エージェントが**普通にある**（5-13）。実測でセッション `11111111` は hooks が31体、
ディスクに7体。`a1a1a1a1a1a1a1a1a`（4.5）も同じ形。
...
`fired` を立てるのが通知で、**再武装（`fired` を落とす）は4つの理由でしか起きない**:
...
4. **マスタースイッチが動いた、またはブラウザの通知許可を得た** —— `rearmQuota()` が
   状態ごと捨て、**直後のスナップショットを `prime` にする**。3 と違って
   **その場では鳴らさない**。3 は「今の枠についての問いが変わった」ので即答するのが
   親切だが、4 は「これから先を教えてくれ」という意思表示で、スイッチを入れた瞬間に
   古い超過が飛び出すのは警告ではなくバグに見える。セッション通知側でも
   ON にした瞬間に溜まっていた分を再生したりはしない（`primeNotifications()`）ので、
   そちらに揃えた。

**鳴らせない間は武装状態を一切動かさない。** `canNotify()` が false のとき
（マスター OFF・権限なし・`quietWhenFocused` で抑制中）、`evaluateQuota()` は
`allowed: false` を受け取って**受け取った `armed` をそのまま返す**。
以前は毎スナップショット評価していたので、**誰も見ていない超過で `fired` が
立ち切ってしまい**、あとから ON にしても `resets_at` が転がるまで
（`seven_day` なら最大1週間）鳴らなかった。沈黙は「対応済み」ではない。

なお `quietWhenFocused` による抑制もこの凍結に含まれる。画面を見ている間に
80% を跨いだ場合、その事実はリボンに出ているが `fired` は立たず、
**タブから離れた最初のスナップショットで鳴る**。「見ている間は黙る」であって
「見ている間に起きたことは無かったことにする」ではない。

**ただし `prime` だけは黙っていても実行する**（`allowed || quotaPrime`）。
prime は構造上1件も鳴らさないので凍結する理由が無く、逆に「最初に鳴らせる
スナップショットまで prime を先送りする」と、**そのスナップショットが prime に
消費されて、凍結して守ったはずの超過をそこで飲み込む**。
prime が取るのは「我々が見始めた時点の基準」であって、それは鳴らせるかどうかとは
別の話である。
...
- **`disabled` は毎回計算する。** マスタースイッチのボタンは `perm === 'denied'` を
  そのまま代入する。以前は `denied` を見たときに `disabled = true` を立てるだけで
  落とす箇所が無く、**ブラウザ側でブロックを解除してもタブを再読込みするまで
  押せないまま**だった。表示テキストも同じ式から出す。

### 9.6 どこから鳴らすか —— 通知リングの読み方

セッション通知（`fireNotifications()`）は「前回反応した id」を覚えて、
その次から鳴らす。サーバは1セッションあたり **20件のリング**しか持たず
（`MAX_NOTIFICATIONS`）、id は `<sessionId>#<連番>` でプロセス内カウンタから採る。
この2つから、覚えている id が**リストに無い**ことが普通に起きる:

- タブが寝ている・切断している間にリングから押し出された。
- サーバが再起動してカウンタが 1 に戻った。

以前はどちらの場合も先頭（index 0）から鳴らしていたので、**1スナップショットで
最大20件が一斉に飛ぶ**——priming が避けたかったバーストそのものだった。
いまは `CMNotifyRules.nextNotificationStart()` が判断し、**見つからなければ
最新1件だけ**を鳴らして同期を取り直す。多すぎるより少なすぎる方がましである。

同じ理由の裏返しで、`Map.get()` の `undefined`（**知らないセッション**）と
`null`（**知っているが通知が無かったセッション**）を区別する。前者は届いた
バックログを全部鳴らす筋合いが無いので最新1件、後者は全件が本当に新着である。

再起動でカウンタが戻り、**古い id が別の通知にたまたま一致してしまう**ケースだけは
クライアント側から見分けがつかない（サーバを触らずに済ませたかったので、
id に起動ごとのランダム接頭辞を足すような変更はしていない）。
その場合は一致点より前を静かに飛ばす。取りこぼす方に倒した、既知の妥協である。
